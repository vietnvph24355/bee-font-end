...
import { CreatedRequest } from "~/interfaces/diaHinhSan.type";
...
const add: React.FC = () => {
  ...
  const onFinish = (values: CreatedRequest) => {
    confirm({
      ...
      content: "Bạn có chắc thêm địa hình sân này không?",
      okText: "OK",
      cancelText: "Hủy",
      onOk: async () => {
        try {
          setLoading(true);
          await request.post("dia-hinh-san", {
            ten: values.ten,
          });
          setLoading(false);
